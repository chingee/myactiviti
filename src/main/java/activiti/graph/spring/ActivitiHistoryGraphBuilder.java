...
import activiti.graph.Edge;
import activiti.graph.Graph;
import activiti.graph.Node;
...
public class ActivitiHistoryGraphBuilder {
    ...
    public Graph build() {
        ...
        for (int i=0; i<historicActivityInstances.size(); i++) {
            ...
            if (previousEdge == null) {
                if (graph.getInitial() != null) {
                    throw new IllegalStateException("已经设置了一个开始节点");
                }
        ...
        if (graph.getInitial() == null) {
            throw new IllegalStateException("找不到开始节点");
        }
    ...
    public Edge findPreviousEdge(Node currentNode, long currentStartTime) {
        ...
        for (PvmTransition pvmTransition : activityImpl
            ...
            /**
             * 这个逻辑不对, 源api中, 当前节点
             * 	1.如果是上一个节点是userTask, 则当前节点开始时间<=上一个节点结束时间
             *  2.只有当前节点是网关时, 才符合此逻辑
             */
            if (currentStartTime < visitiedHistoryActivityInstance.getEndTime()
                    .getTime() && !currentNode.getType().equals(BpmnXMLConstants.ELEMENT_TASK_USER)) {
                continue;
            }
