...
//				+ '<div><b>isHighLighted</b>: {isHighLighted}</div>';
		var TPL_FLOW_INFO = 
			  '<div><b>名称</b>: {name}</div>'
			+ '<div><b>源节点标识</b>: {sourceActivityId}</div>'
			+ '<div><b>指向节点标识</b>: {destinationActivityId}</div>'
			+ '<div><b>流转条件</b>: {isConditional}</div>';
...
//			+ '<div><b>processDefinitonKey</b>: {processDefinitonKey}</div>';
		var TPL_ACTIVITY_INFO = '' 
				...
				+ '<div><b>节点名称</b>: {name}</div>'
				+ '<div><b>审核人信息</b>:'
				+ '<ui><li>aaa</li><li>bbb</li></ui>'
				+ '</div>';
		var TPL_CALLACTIVITY_INFO = ''
				+ '<div><b>collapsed</b>: {collapsed}</div>'
				+ '<div><b>processDefinitonKey</b>: {processDefinitonKey}</div>';
